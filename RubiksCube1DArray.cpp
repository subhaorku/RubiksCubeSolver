...
private:
    ...
    /*
     * This is often done in programming to flatten multidimensional arrays into a one-dimensional array,
     * which can sometimes simplify access and manipulation of data.
     */
    static inline int getIndex(int ind,int row,int col) {
        return (ind*9)+(row*3)+(col);
    }
/*
 * The inline keyword is a suggestion to the compiler to replace a function call with the actual code of the function.
 * This process is called "inlining." By doing this, the overhead associated with a function call (like pushing parameters onto the stack, jumping to the function code, and then returning) is eliminated.
 * Instead, the code of the function is directly inserted where the function is called.
 */
/*
 * static keyword gives this function an internal linkage
 * this means ,getIndex function is visible only in thi file
 * this prevents name conflicts during linking
 */
